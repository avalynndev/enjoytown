...
export async function fetchTvCarousalData(type: string) {
  try {
    const url = new URL(
      `${process.env.GOOD_PROXY_URL}/fetch?url=https://api.themoviedb.org/3/trending/tv/day?api_key=${API_KEY}`
    );
...
export async function fetchCarousalData(type: string) {
  try {
    const url = new URL(
      `${process.env.GOOD_PROXY_URL}/fetch?url=https://api.themoviedb.org/3/trending/movie/day?api_key=${API_KEY}`
    );
    const response = await fetch(url.toString(), {
      next: { revalidate: 60 * 60 * 24 * 7 },
    });
